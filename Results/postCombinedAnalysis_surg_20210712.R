...
# raw_in = readLines("C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/orig_st_results/SickKids_origST_FineTuneUIowa_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json")

  ## all combined
raw_in = readLines("C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/orig_st_results/CombinedInst60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json")
  ## SK-UI combined
# raw_in = readLines("C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/orig_st_results/Combined_SKUI_Inst60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json")
  ## SK-ST combined
# raw_in = readLines("C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/orig_st_results/Combined_SKST_Inst60%_TrainOnly_40epochs_bs16_lr0.001_RCFalse_covTrue_OSFalse.json")


test = v8()
test$assign("dat", JS(raw_in))
st_stan_test = test$get("dat")
st_stan_test = add_platt_preds(in_list = st_stan_test, epoch = 1, sensitivity = SENSITIVITY)
...
st_stan_test = add_platt_preds(in_list = st_stan_test, epoch = 20,sensitivity = SENSITIVITY)
...
st_preds = data.frame(st_pred_mod_ep1 = st_stan_test$st[[1]][[1]]$pred_scaled,
                      st_pred_mod_ep10 = st_stan_test$st[[1]][[10]]$pred_scaled,
                      st_pred_mod_ep13 = st_stan_test$st[[1]][[13]]$pred_scaled,
                      st_pred_mod_ep16 = st_stan_test$st[[1]][[16]]$pred_scaled,
                      st_pred_mod_ep20 = st_stan_test$st[[1]][[20]]$pred_scaled,
...
(dat_thresh_ep1 = st_stan_test[["val"]][[fold]][[1]][["threshold"]])
ggplot(st_preds, aes(x = target, y = st_pred_mod_ep1, fill = target)) + geom_violin() + 
...
ggplot(st_preds, aes(x = target, y = st_pred_mod_ep20, fill = target)) + geom_violin() + 
  geom_hline(yintercept = dat_thresh_ep20) + ylim(0,1) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Prediction")
...
stan_preds = data.frame(stan_pred_mod_ep1 = st_stan_test$stan[[1]][[1]]$pred_scaled,
                        stan_pred_mod_ep10 = st_stan_test$stan[[1]][[10]]$pred_scaled,
                        stan_pred_mod_ep13 = st_stan_test$stan[[1]][[13]]$pred_scaled,
                        stan_pred_mod_ep16 = st_stan_test$stan[[1]][[16]]$pred_scaled,
                        stan_pred_mod_ep20 = st_stan_test$stan[[1]][[20]]$pred_scaled,
...
(dat_thresh_ep10 = st_stan_test[["val"]][[fold]][[1]][["threshold"]])
ggplot(stan_preds, aes(x = target, y = stan_pred_mod_ep1, fill = target)) + geom_violin() + 
  geom_hline(yintercept = dat_thresh_ep10, lwd = 1.2, lty = 2, col = "red") + ylim(0,1) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Prediction")
...
(dat_thresh_ep20 = st_stan_test[["val"]][[fold]][[20]][["threshold"]])
ggplot(stan_preds, aes(x = target, y = stan_pred_mod_ep20, fill = target)) + geom_violin() + 
  geom_hline(yintercept = dat_thresh_ep20, lwd = 1.2, lty = 2, col = "red") + ylim(0,1) + 
  geom_point() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Prediction")
...
ui_preds = data.frame(ui_pred_mod_ep1 = st_stan_test$ui[[1]][[1]]$pred_scaled,
                      ui_pred_mod_ep10 = st_stan_test$ui[[1]][[10]]$pred_scaled,
                      ui_pred_mod_ep13 = st_stan_test$ui[[1]][[13]]$pred_scaled,
                      ui_pred_mod_ep16 = st_stan_test$ui[[1]][[16]]$pred_scaled,
                      ui_pred_mod_ep20 = st_stan_test$ui[[1]][[20]]$pred_scaled,
...
(dat_thresh_ep10 = st_stan_test[["val"]][[fold]][[1]][["threshold"]])
ggplot(ui_preds, aes(x = target, y = ui_pred_mod_ep1, fill = target)) + geom_violin() + 
  geom_hline(yintercept = dat_thresh_ep10, lwd = 1.2, lty = 2, col = "red") + ylim(0,1) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Prediction")
...
(dat_thresh_ep20 = st_stan_test[["val"]][[fold]][[20]][["threshold"]])
ggplot(ui_preds, aes(x = target, y = ui_pred_mod_ep20, fill = target)) + geom_violin() + 
  geom_hline(yintercept = dat_thresh_ep30, lwd = 1.2, lty = 2, col = "red") + ylim(0,1) + 
  geom_point() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + ylab("Prediction")
...
saveRDS(st_hn_only, file = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ST_CombinedTrain_20210818.rds")

### most recent only dataset: 
hn_only_most_recent = get_most_recent(in_dataset = st_hn_only)
head(hn_only_most_recent)
saveRDS(hn_only_most_recent, file = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/STmostrecent_CombinedTrain_20210818.rds")
...
## overall
get_sen_spec_withCI(score_vec = st_hn_only$st_pred_mod_ep20, 
                    target = st_hn_only$target01, 
                    threshold = dat_thresh_ep20, 
                    n_it = 500, out_vecs = FALSE)

## most recent only 
get_sen_spec_withCI(score_vec = hn_only_most_recent$st_pred_mod_ep20, 
                    target = hn_only_most_recent$target01, 
                    threshold = dat_thresh_ep20, 
                    n_it = 500, out_vecs = FALSE)
...
saveRDS(stan_merge, file = "C:/Users/lauren erdman/OneDrive - SickKids/HN/Stanford/Stanford_CombinedTrain_20210818.rds")


## overall -- all kidneys
get_sen_spec_withCI(score_vec = stan_df$stan_pred_mod_ep1, 
...
get_sen_spec_withCI(score_vec = stan_merge$stan_pred_mod_ep1, 
...
get_sen_spec_withCI(score_vec = stan_merge$stan_pred_mod_ep20, 
                    target = stan_merge$target01, 
                    threshold = dat_thresh_ep20, 
                    n_it = 500)
...
saveRDS(ui_df, file = "C:/Users/lauren erdman/OneDrive - SickKids/HN/UIowa/UI_CombinedTrain_20210818.rds")

## overall 
get_sen_spec_withCI(score_vec = ui_df$ui_pred_mod_ep1,
...
get_sen_spec_withCI(score_vec = ui_df$ui_pred_mod_ep20, 
                    target = ui_df$target01, 
                    threshold = dat_thresh_ep20, 
                    n_it = 500)
